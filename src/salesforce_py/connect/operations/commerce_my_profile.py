...
class CommerceMyProfileOperations(ConnectBaseOperations):
    ...
    def _base(self, webstore_id: str) -> str:
        return f"commerce/webstores/{self._ensure_18(webstore_id)}/myprofile"
    ...
    async def update_profile(self, webstore_id: str, body: dict[str, Any]) -> dict[str, Any]:
    ...
    async def init_otp(self, webstore_id: str, body: dict[str, Any]) -> dict[str, Any]:
        """Initiate a one-time password (OTP) via SMS or email.

        Args:
            body: Buyer Profile Initiate OTP Input payload.
        """
        return await self._post(f"{self._base(webstore_id)}/actions/initOTP", json=body)

    async def verify_otp(self, webstore_id: str, body: dict[str, Any]) -> dict[str, Any]:
        """Validate the OTP provided by the buyer.

        Args:
            body: Buyer Profile Verify Input payload.
        """
        return await self._post(f"{self._base(webstore_id)}/actions/verify", json=body)
