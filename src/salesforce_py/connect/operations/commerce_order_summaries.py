...
class CommerceOrderSummariesOperations(ConnectBaseOperations):
    ...
    def _base(self, webstore_id: str) -> str:
        return (
            f"commerce/webstores/{self._ensure_18(webstore_id)}/order-summaries"
        )
    ...
    async def get_order_summary(
        self, webstore_id: str, order_summary_id: str
    ) -> dict[str, Any]:
    ...
    async def list_delivery_groups(
        ...
        return await self._get(
            f"{self._base(webstore_id)}/{order_summary_id}/delivery-groups"
        )
    ...
    async def list_items(
        ...
        order_summary_id = self._ensure_18(order_summary_id)
        return await self._get(
            f"{self._base(webstore_id)}/{order_summary_id}/items"
        )
    ...
    async def list_shipments(
        ...
        order_summary_id = self._ensure_18(order_summary_id)
        return await self._get(
            f"{self._base(webstore_id)}/{order_summary_id}/shipments"
        )
    ...
    async def lookup(
        ...
        return await self._post(
            f"{self._base(webstore_id)}/actions/lookup", json=body
        )
    ...
    async def authorize_guest(
        ...
        return await self._post(
            f"{self._base(webstore_id)}/actions/authorize", json=body
        )

    async def list_shipment_items(
        self, webstore_id: str, shipment_id: str
    ) -> dict[str, Any]:
        """Get shipment items."""
        shipment_id = self._ensure_18(shipment_id)
        return await self._get(
            f"commerce/webstores/{self._ensure_18(webstore_id)}"
