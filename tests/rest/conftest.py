"""Shared fixtures + helpers for the REST API test suite."""
...
def make_response(
    ...
    *,
    ...
    """Build a mock ``httpx.Response`` for REST API tests.

    Mirrors the pattern used in tests/bulk/conftest.py — a MagicMock that
    exposes the attributes the client reads from a real response
    ...
    resp.url = f"{INSTANCE_URL}/services/data/v{API_VERSION}/"
...
async def make_client(
    *,
    ...
    """Open a :class:`RestClient` and replace its session verbs with AsyncMocks."""
    from salesforce_py.rest.client import RestClient

    client = RestClient(INSTANCE_URL, ACCESS_TOKEN, api_version=API_VERSION)
    await client.open()
