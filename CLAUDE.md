...
salesforce-py ships three independent clients that target different Salesforce surfaces. They share `salesforce_py.exceptions` and `salesforce_py.utils`, but are otherwise unrelated:
...
- `AuthError` is raised by both `ConnectBaseOperations._handle_status` and `Data360BaseOperations._handle_status` on a 401 response.
...
- `dev` — `pytest`, `pytest-asyncio`, `pytest-cov`, `ruff`, `ty`, and `httpx[http2]` (so the Connect + Data 360 tests run under `dev`).
